...
- **` #lumen/opencli `**: Triggers the **OpenCLI Tool Bus**. Run local shell commands and system scripts.
- **`to_canvas_edge` (Action)**: Used in Skills to let AI automatically draw logical connections.

![LUMEN Operators Showcase](./image/CleanShot%202026-04-11%20at%2013.21.00.png)
...
- **` #lumen/opencli `**：**控制台算子** (OpenCLI)。在白板内运行本地 Shell 命令与系统脚本。
- **`to_canvas_edge` (逻辑算子)**：通常用于轻技能中，允许 AI 分析多个选中节点并自动建立逻辑连线。

![LUMEN 算子全览](./image/CleanShot%202026-04-11%20at%2013.21.00.png)
