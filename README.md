...
- **Spatial Reasoning**: Convert notes to spatial whiteboards with AI-powered semantic layout.
...
- **Smart Annotation**: Mandatory context check on indexing to ensure your personal insights are weighted properly.
...
- **智能批注**：强制性的入库备注机制，确保个人感悟在搜索中占据最高权重。
- **灵感召唤 (Summoning)**：在白板中通过语义相似度自动拉取并连线呈现关联笔记。

##### 4. 多模态创作与连接
打破文本、图像、视频与现实世界的界限。
- **白板算子**：原生支持 `#lumen/image`, `#lumen/video`, `#lumen/audio` 标签。
- **视觉反向工程**：使用 `#lumen/image2prompt` 从参考图中提取结构化提示词。
- **OpenCLI 桥接**：原生集成 OpenCLI 工具链，支持在 Obsidian 内驱动本地脚本并将结果直接渲染至白板。
- **联网搜索**：集成 Serper/Tavily，让 AI 拥有实时互联网调研能力。
