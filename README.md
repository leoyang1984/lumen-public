...
- **Immediate Inline Editing**: Type `/lumen` directly in the editor to pop up a native menu of your Light Skills. 
...
- **Default Action**: Set your favorite skill to trigger instantly when you press `Enter` after `/lumen`.
...
- Supports any OpenAI-compatible API. Native optimizations for **Lumen** and **Kimi**.
...
- **AI Refinement**: Automatically correct typos, remove filler words, and format into structured Markdown using Lumen.
...
- **Node-Based Programming**: Turn Obsidian Canvas into a no-code AI workflow engine. Create `#lumen/input` and `#lumen/step` cards and connect them with arrows to build multi-step Agentic pipelines visually.
- **One-Click Execution**: Add a `#lumen/start` block to render a "Run Flow" button. Click it to automatically sequentially extract variables, merge contexts, and process your AI workflow.
...
- **Multimodal Creation**: Add `#lumen/image [prompt]` to any Canvas node to enable manual AI image generation.
...
- **Visual Synthesis**: Add `#lumen/video` to any node to trigger AI video generation.
...
- **Reverse-Engineer Inspiration**: Add `#lumen/image2prompt` to any node to extract detailed, editable prompts from reference images.
...
2. **Directory**: Place files in `.obsidian/plugins/lumen-obsidian-plugin/`.
...
- **v2.8.1 (2026-03-27)**: **Image-to-Image (Img2Img) Support** — Introduced the ability to use existing Canvas images as reference for AI generation. Simply connect an image node and a prompt node to the `#lumen/image` operator to trigger transformation.
...
- **v2.5.5 (2026-03-23)**: **Canvas Flow** — A fully functional visual workflow engine inside Obsidian Canvas. Build multi-step Agentic pipelines using `#lumen/start`, `#lumen/input/n`, and `#lumen/step/n` tags. Features topology-aware execution (Kahn's algorithm) for parallel branches, real-time node highlighting, variable resolution, and automatic shadow node generation.
...
- **v1.7.0 (2026-03-20)**: Added native **Slash Command (`/lumen`)** system for in-place text replacement.
...
- 支持任何兼容 OpenAI 格式的 API。针对 **Lumen** 和 **Kimi** 进行了原生优化。
...
- **AI 自动润色**：利用 Lumen 自动修正口语错别字并转化为整齐的 Markdown。
...
- **节点化编程**：把 Obsidian 白板变成一个不用写代码的 AI 工作流引擎。只需建立几个卡片，打上 `#lumen/input` 和 `#lumen/step` 标签并用箭头相连，就能画出复杂流水线。
- **一键全自动**：新建 `#lumen/start` 触发节点即可渲染出紫色的“Run Flow”按钮，点击后 AI 会沿连线自动汇聚变量、合并不同输入分支并执行。
...
- **多模态创作**：在任何白板节点中添加 `#lumen/image [提示词]` 即可开启手动 AI 图像生成。
...
- **视觉合成增强**：在节点中添加 `#lumen/video` 即可触发视频生成。
...
#### 17. 🎨 视觉提示词提取 (v3.0.1)
- **视觉反向工程**：在节点中添加 `#lumen/image2prompt` 标签，即可将参考图解析为结构化、可编辑的提示词。
- **七轴深度分析**：自动从艺术风格、构图、光影、角色、材质、氛围、色彩等 7 个维度进行解析。
- **创作闭环**：通过该算子获取提示词并进行微调后，可直接连入 `#lumen/image` (生图) 或 `#lumen/video` (生视频) 算子。
...
2. **存放目录**：放在 `.obsidian/plugins/lumen-obsidian-plugin/` 下。
...
- **v2.8.1 (2026-03-27)**：**图生图 (Img2Img) 支持**。新增了以白板现有图片为参考进行 AI 创作的能力。只需将图片节点和提示词节点同时连线至 `#lumen/image` 算子，即可触发图像转换。
...
- **v2.7.8 (2026-03-24)**：**多模态图像生成**。引入了 `#lumen/image` 算子，支持在白板中手动创作 AI 图像。适配 **OpenRouter** 与 **SiliconFlow**。具备健壮的 Base64/URL 解析引擎、自动递归目录创建、基于全局搜索的白板节点 ID 解决机制，并完成了全代码库的英文标准化。
...
- **v2.5.5 (2026-03-23)**：**Canvas Flow 可视化 AI 工作流**。将白板变成无代码工作流引擎，通过 `#lumen/start`, `#lumen/input/n`, `#lumen/step/n` 标签构建管道。支持拓扑排序自动处理并行分支、节点实时高亮反馈与自动创建"影子结果"节点。
...
- **v1.7.0 (2026-03-20)**：新增 **Slash Command 斜杠命令 (`/lumen`)** 与基础 UI 联动功能。支持原地文本即刻替换。
